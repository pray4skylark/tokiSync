...
function isNtkNovelSite(siteInfo) {
    try {
        const hostname = new URL(siteInfo.protocolDomain).hostname;
        return siteInfo.category === 'Novel' && /^ntk\d+\.com$/.test(hostname);
    } catch (e) {
        return false;
    }
}
...
function getSingleVolumeFilename(job) {
    const first = job.items[0];
    const last = job.items[job.items.length - 1];
    const startRaw = first ? first.num : "";
    const endRaw = last ? last.num : "";
    ...
    return `${job.seriesTitle || job.rootFolder} (${rangeLabel})`;
}

function clearNovelSingleVolumeJob() {
    if (typeof GM_deleteValue === 'function') {
        GM_deleteValue(NOVEL_SINGLE_VOLUME_JOB_KEY);
        GM_deleteValue(NOVEL_SINGLE_VOLUME_CHAPTERS_KEY);
    }
}
...
async function startNovelSingleVolumeNavigationJob({ list, parser, siteInfo, rootFolder, seriesTitle, destination, category, seriesMetadata }) {
    ...
    if (items.length === 0) {
        throw new Error('단행본 합본 작업을 시작할 회차가 없습니다.');
    }
    ...
    GM_setValue(NOVEL_SINGLE_VOLUME_JOB_KEY, job);
    GM_setValue(NOVEL_SINGLE_VOLUME_CHAPTERS_KEY, []);
    LogBox.getInstance().log(`📙 ntk 단행본 탭 순회 모드 시작 (${items.length}개). 첫 회차로 이동합니다.`, 'Downloader');
    window.location.href = items[0].src;
}

async function waitForTopLevelNovelContent(parser, logger) {
    const MAX_ATTEMPTS = 5;
    const POLL_MS = 500;
    const WAIT_PER_ATTEMPT_MS = 12000;

    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        const maxPolls = Math.ceil(WAIT_PER_ATTEMPT_MS / POLL_MS);

        for (let i = 0; i < maxPolls; i++) {
            const text = parser.getNovelContent(document);
        ...
        const refreshControl = Array.from(document.querySelectorAll('button, a, [role="button"]'))
            .find((el) => /새로고침/.test((el.innerText || el.textContent || '').trim()));

        if (refreshControl) {
            logger.warn(`[Novel:Tab] 본문 미로드 상태, 새로고침 클릭 (${attempt}/${MAX_ATTEMPTS})`, 'Parser');
            refreshControl.click();
        } else {
            logger.warn(`[Novel:Tab] 본문 미로드 상태, 페이지 재로드 (${attempt}/${MAX_ATTEMPTS})`, 'Parser');
    ...
    throw new Error('소설 본문을 불러오지 못했습니다. 상단 회차 페이지 새로고침이 계속 실패했습니다.');
}

export async function resumeNovelSingleVolumeJob(siteInfo) {
    ...
    if (currentIndex < 0) {
        currentIndex = job.index || 0;
        if (job.items[currentIndex]) {
            window.location.href = job.items[currentIndex].src;
            return true;
        }
        clearNovelSingleVolumeJob();
        return false;
    ...
    if (!parser) {
        clearNovelSingleVolumeJob();
        throw new Error('파서를 초기화할 수 없어 단행본 작업을 중단합니다.');
    }

    const item = job.items[currentIndex];
    logger.log(`[Novel:Tab] [${currentIndex + 1}/${job.items.length}] 본문 수집 중: ${item.title}`, 'Downloader');

    try {
        const text = await waitForTopLevelNovelContent(parser, logger);
        if (!text) return true;

        const chapters = GM_getValue(NOVEL_SINGLE_VOLUME_CHAPTERS_KEY, []);
        chapters[currentIndex] = {
            title: item.title,
            text
        };
        GM_setValue(NOVEL_SINGLE_VOLUME_CHAPTERS_KEY, chapters);
        logger.success(`[Novel:Tab] 챕터 저장 완료: ${item.title}`, 'Downloader');

        if (currentIndex < job.items.length - 1) {
            job.index = currentIndex + 1;
            GM_setValue(NOVEL_SINGLE_VOLUME_JOB_KEY, job);
            window.location.href = job.items[currentIndex + 1].src;
            return true;
        }

        if (chapters.filter(Boolean).length !== job.items.length) {
            throw new Error('수집된 챕터 수가 요청 범위와 맞지 않아 결손 EPUB 생성을 중단합니다.');
        }

        const builder = new EpubBuilder();
        chapters.forEach((chapter) => builder.addChapter(chapter.title, chapter.text));

        const finalFilename = getSingleVolumeFilename(job);
        logger.log(`📚 단행본 조립 및 저장 중... (${finalFilename})`, 'Downloader');

        const finalZip = await builder.build({
            series: job.seriesTitle || job.rootFolder,
            title: job.seriesTitle || job.rootFolder,
            writer: job.site
        });
        const finalBlob = await finalZip.generateAsync({ type: "blob" });

        await saveFile(finalBlob, finalFilename, job.destination, 'epub', {
            folderName: job.rootFolder,
            category: job.category
        });

        if (job.destination === 'drive') {
            refreshCacheAfterUpload(job.rootFolder, job.category, job.metadata || {}).catch(e =>
                logger.warn(`캐시 갱신 호출 중 실패 (무시): ${e.message}`, 'GAS:Cache')
            );
        }

        clearNovelSingleVolumeJob();
        logger.success(`✅ 단행본 합본 저장 완료: ${finalFilename}`, 'Downloader');
        Notifier.notify('TokiSync', `단행본 합본 저장 완료! (${job.items.length}개 항목)`);
        return true;
    } catch (err) {
        clearNovelSingleVolumeJob();
        logger.error(`[Novel:Tab] 단행본 합본 작업 중단: ${err.message}`, 'Downloader');
        alert(`단행본 합본 작업 중단:\n${err.message}`);
        return true;
    }
}
...
export async function tokiDownload(rangeSpec, policy = 'zipOfCbzs', forceOverwrite = false) {
    ...
    try {
        ...
        const activeNovelMode = getConfig().novelMode;
        const shouldUseTabNovelJob = isNovel && activeNovelMode === 'singleVolume' && isNtkNovelSite(siteInfo);
        if (shouldUseTabNovelJob) {
            await startNovelSingleVolumeNavigationJob({
                list,
                parser,
                siteInfo,
                rootFolder,
