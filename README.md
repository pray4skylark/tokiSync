# ⚡️ TokiSync (토끼싱크) v1.24.0

**북토끼, 뉴토끼, 마나토끼**의 콘텐츠를 **구글 드라이브로 직접 업로드**하고, **전용 웹 뷰어**를 통해 편리하게 관리/열람할 수 있는 올인원 솔루션입니다.

> **🚀 v1.24.0 업데이트 요약:**
> - **소스벤더(vendor) 전파 체인 연동**: 클라이언트 파서부터 구글 드라이브 스토리지의 info.json 및 메타데이터 색인까지 소스벤더 식별자를 온전히 전파하여 구조적 식별성을 고도화했습니다.
> - **데드락 방지 Liveness Guard 구현**: 팝업창 수동 종료 및 비정상 크래시 발생 시 1초/2초 주기로 이를 Failsafe 감지하여 세마포어 락 동결 없이 즉시 다음 대기열로 복구/전진시킵니다.

---

## ✨ 주요 기능

### 📥 수집기 (UserScript) - v1.24.0
