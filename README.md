# ⚡️ TokiSync (토끼싱크) v1.9.4

**북토끼, 뉴토끼, 마나토끼**의 콘텐츠를 **구글 드라이브로 직접 업로드**하고, **전용 웹 뷰어**를 통해 편리하게 관리/열람할 수 있는 올인원 솔루션입니다.

> **🚀 v1.9.4 업데이트 요약:**
> **지능형 제너릭 파서 고도화 및 광고 이미지 완전 격리**: 도메인 로테이션(예: `sbxh1.com` 등)에 유연하게 대응하기 위해 **실시간 RegExp 자가 조립** 방식을 도입하여 수동 매칭 부담을 없앴습니다. 또한, `idMatch`가 지정되지 않은 경우에도 카테고리 기반 분석을 통해 **작품 고유 ID를 안전하게 자동 추출(Fallback ID)** 해내며, `imageRegex`와 `imageContainer` 간의 동작 정합성을 규정하여 **광고 이미지가 뷰어 본문에 대량 노출되던 문제를 근본적으로 해결**했습니다.

---

## ✨ 주요 기능

### 📥 수집기 (UserScript) - v1.9.4
...
### 📊 뷰어 2.0 (Cinematic & Refined) - v1.9.4
