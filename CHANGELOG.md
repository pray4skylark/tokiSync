...
### ✨ tokiSync 뷰어 편의 기능 대폭 개선 및 오프라인 관리 고도화 (Viewer v1.22.0)
- **라이브러리 3종 정렬 필터 도입**: 최근읽은순(최근 열람 기록 기준), 최신 업데이트순(드라이브 갱신일 기준), 가나다순 정렬 기능을 추가하여 컬렉션 탐색 사용성을 대폭 개선했습니다.
- **5방향 터치 커스터마이징 및 즉시 이동**: 상/하/좌/우/중앙의 5개 영역에 이전/다음/메뉴 토글 동작을 자유롭게 매핑하고, 페이지 전환 애니메이션 딜레이가 없는 즉각 이동(behavior: auto) 모드를 지원합니다.
- **IndexedDB 오프라인 스토리지 GC 패널**: 로컬 브라우저에 캐시된 에피소드 목록과 스토리지 점유 용량을 시각화하고, 개별 또는 일괄 수동 삭제가 가능한 캐시 정리 기능을 구현했습니다.
- **작품 메타데이터 수정 및 썸네일 직접 업로드**: 뷰어 내에서 작품명, 작가, 연재상태, 카테고리, 요약을 수정할 수 있으며, 썸네일 이미지를 업로드하면 Base64 디코딩을 거쳐 구글 드라이브에 직접 파일로 저장 및 색인화(index.json)되도록 GAS 백엔드를 확장했습니다.

## [v1.22.1] - 2026-06-07

### 🛠️ 대기열 중복 수집 간섭 패치 및 런타임 오류 핫픽스
- **대기열 프리체크 스마트 필터 추가**: 새 수집 기동 시 기존 대기열에 실패(`failed`)나 미완료 항목이 있는 경우 안내창(`confirm`)을 띄우며, 취소 시 요청을 중단하고 대시보드 대기열 창을 자동으로 노출하여 사용자가 수동으로 제어하도록 개선했습니다. (성공 완료된 큐만 있을 시 자동 초기화 수행)
