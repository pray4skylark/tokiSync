...
- **본문 오염 방지**: ntk 소설의 `본문을 불러올 수 없습니다` 화면이나 댓글 영역을 EPUB 본문으로 오인하지 않도록 필터링하고, iframe 내 새로고침 폴백을 추가했습니다.
- **합본 무결성 보장**: 소설 단행본 합본 모드에서는 회차 하나라도 실패하면 결손 EPUB을 저장하지 않고 전체 작업을 중단하도록 변경했습니다.
- **메뉴 버튼 복구**: ntk 페이지의 지연 렌더링이 TokiSync 플로팅 버튼을 제거하는 경우를 감지해 자동으로 다시 붙이도록 보강했습니다.
- **ntk 합본 탭 순회 모드**: iframe 접근이 막히는 ntk 소설 합본 다운로드는 현재 탭을 회차별로 이동하며 상위 페이지 본문을 읽고, 마지막 회차에서 EPUB을 조립하도록 변경했습니다.
- **합본 작업 취소 안전장치**: 탭 순회 합본 작업 상태를 수동 취소할 수 있도록 로그창 STOP 버튼과 작업 만료/취소 플래그를 추가했습니다.
